...
export default function AgentenPage() {
  return (
    <div className="acc-page min-h-screen text-[#2a2521]">
      ...
      <main className="mx-auto w-full max-w-6xl px-4 pb-24 pt-6 sm:px-6">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-[15px] leading-relaxed text-[#5c5346]">
          ...
          <div className="mt-5 flex flex-wrap gap-2 text-[12px]">
            <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[#4a4335]">
              {ABTEILUNGEN.length} Abteilungen
            </span>
            <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[#4a4335]">
              {AGENT_ANZAHL} benannte Agenten
            </span>
            <span className="rounded-full border border-[#ff8c2a]/30 bg-[#ff8c2a]/[0.08] px-3 py-1 text-[#c25e0e]">
            ...
            return (
              <section key={abt}>
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <span className="font-mono text-[11px] text-[#8d8172]">{agenten.length}</span>
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {agenten.map((a) => (
                    <div
                      key={a.id}
                      className="group rounded-2xl border border-white/8 bg-white/[0.03] p-4 transition-colors hover:border-white/15 hover:bg-[#1c1917]/[0.04]"
                      style={{ borderTopColor: akzent, borderTopWidth: 2 }}
                    >
                      <div className="flex items-center gap-2">
                        <h3 className="text-[14px] font-semibold text-[#1c1917]">{a.name}</h3>
                        {a.geplant && (
                          <span className="rounded-full border border-zinc-500/40 px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wider text-[#5c5346]">
                            geplant
                          </span>
                        )}
                      </div>
                      <p className="mt-1.5 text-[13px] leading-relaxed text-[#5c5346]">{a.aufgabe}</p>
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {a.kann.map((k) => (
                          <span
                            key={k}
                            className="rounded-md bg-[#1c1917]/[0.04] px-2 py-0.5 text-[11px] text-[#4a4335]"
                          >
                            {k}
                          </span>
                        ))}
        ...
        <div className="mt-12 rounded-2xl border border-[#ff8c2a]/25 bg-gradient-to-br from-[#ff8c2a]/[0.08] to-transparent p-6 text-center">
          ...
          <p className="mx-auto mt-2 max-w-xl text-[14px] text-[#5c5346]">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-3">
            <Link
              href="/dashboard"
              className="rounded-xl bg-gradient-to-br from-[#ff8c2a] to-[#ff5f1f] px-5 py-2.5 text-[14px] font-semibold text-[#1c1917] shadow-[0_6px_20px_-8px_rgba(255,110,30,0.6)]"
            >
              Mission starten
            </Link>
            <Link
              href="/assistent"
              className="rounded-xl border border-white/12 bg-white/[0.04] px-5 py-2.5 text-[14px] font-medium text-[#2a2521] hover:bg-white/[0.07]"
            >
              KI-Chat öffnen
            </Link>
          </div>
