 ...
 * { text, pages }. Wird vom Dashboard fuer die Dokumenten-Analyse
...
/** Harte Obergrenze fuer den PDF-Upload (Vercel-tauglich). */
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    return jsonError("Ungueltiger Request-Body (multipart/form-data erwartet).", 400);
  }
  ...
  // PDF-Magic-Bytes pruefen statt dem Client-MIME-Type zu vertrauen.
  if (!isPdf(data)) {
    return jsonError("Nur PDF-Dateien werden unterstuetzt.", 415);
  }

  const parser = new PDFParse({ data });
  try {
    // pageJoiner "\n": Seiten nur durch Zeilenumbruch trennen statt der
    // Standard-Markierung "-- 1 of n --" (stoert die Dokumenten-Analyse).
    ...
    return jsonError(
      "Das PDF konnte nicht gelesen werden (beschaedigt oder verschluesselt?).",
      422,
    );
  } finally {
...
/** Prueft die "%PDF-"-Signatur am Dateianfang. */
